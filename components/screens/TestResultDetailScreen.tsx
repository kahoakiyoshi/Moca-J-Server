...
const TestResultDetailScreen: React.FC<TestResultDetailScreenProps> = ({ id, onBack }) => {
  ...
  const items = useMemo(() => {
    ...
    return testResult.result.map((item: any, idx: number) => ({
      no: item.questionKey || `${idx + 1}`,
      name: item.taskKey || 'Unknown Task',
      criterion: '-',
      result: 'OK', // Basic placeholder
      autoScore: '-',
      score: '-',
      mocaJ: '○',
      time: item.timestamp ? new Date(item.timestamp).toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' }) : '-',
      note: JSON.stringify(item.value),
      btn: '確認',
      type: item.taskKey
    }));
  }, [testResult]);
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-500 pb-10">
        ...
        <Table className="text-[13px]">
          <TableHeader className="bg-neutral-50">
            <TableRow className="hover:bg-transparent">
              ...
              <TableHead className="hidden lg:table-cell font-bold text-neutral-500 uppercase text-[10px]">判定基準</TableHead>
              <TableHead className="text-center font-bold text-neutral-500 uppercase text-[10px]">結果</TableHead>
              <TableHead className="text-center font-bold text-neutral-500 uppercase text-[10px]">自動</TableHead>
            ...
            {items.map((item: any, idx: number) => (
              <TableRow key={idx} className="hover:bg-neutral-50/50 transition-colors">
                ...
                <TableCell className="font-bold text-neutral-800">
                  {alls_test[`${item.name}_${item.no}`] || alls_test[item.name] || item.name}
                </TableCell>
                <TableCell className="hidden lg:table-cell text-neutral-400 text-[11px]">{item.criterion}</TableCell>
                <TableCell className="text-center whitespace-nowrap">
                  <Badge variant={item.result === 'OK' ? 'outline' : 'destructive'}
                    className={`rounded-full px-2 leading-none h-5 text-[10px] font-bold ${item.result === 'OK' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-red-50 text-red-600 border-red-100 hover:bg-red-50'}`}>
                    {item.result}
                  </Badge>
                </TableCell>
                <TableCell className="text-center font-semibold text-neutral-500">{item.autoScore}</TableCell>
                ...
                <TableCell className="text-center">
                  <span className={`inline-block w-5 h-5 rounded-full text-center leading-5 text-[11px] font-bold 
                    ${item.mocaJ === '○' ? 'bg-emerald-100 text-emerald-700' :
                      item.mocaJ === '△' ? 'bg-amber-100 text-amber-700' :
                        'bg-red-100 text-red-700'}`}>
                    {item.mocaJ}
                  </span>
                </TableCell>
                <TableCell className="text-neutral-500 font-medium">{item.time}</TableCell>
